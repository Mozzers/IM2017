...
public class ConnectRsp {
	...
	private byte[] compat_high = new byte[1];
	private byte[] compat_low = new byte[1];
	private byte[] error = new byte[1];
	private byte[] return_ = new byte[1];
	
	public ConnectRsp(TransHd trans_hd, byte[] window_size, byte[] compat_high, byte[] compat_low, byte[] error, byte[] return_) {
		super();
